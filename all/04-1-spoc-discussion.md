...
1. 寄存器、高速缓存、内存、外存的访问特征？
   ...
   **寄存器：空间最小，速度最快，价格最贵，每一次时钟脉冲可改变一次值**

   **高速缓存：介于寄存器与内存之间，用于存放常被cpu访问的内存内容，以提高存储与cpu之间的沟通速度**

   **内存：正在运行程序的代码和数据主要存储在内存，内存断电后数据丢失。**

   **外存：速度最慢、空间最大、价格最便宜，用于长期保存数据，断电后数据仍然存在**

2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？

   **每一级存储器都尽可能发挥全部的速度，不会存在某一级存储器过度繁忙，而使上下级存储器进行等待**
...
1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
   ...
   **不存在相互调用关系的程序块可以相互覆盖而使用同一块内存空间，目的对于只有一个程序存在内存时节省内存空间**

2. 什么是交换技术？覆盖与交换有什么不同？

   **将暂时不能运行的进程换出，换入某个外存中的进程到内存。不同在于交换用于解决多个程序存在内存时内存不足，而覆盖是针对只有一个程序的情况**

3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？

   **覆盖可用于嵌入式系统，交换可用于内存临时不够用**

4. 如何分析内核模块间的依赖关系？

   **lsmod指令，查看modules.dep，modinfo命令**
...
1. 什么是时间局部性、空间局部性和分支局部性？
   ...
   **时间局部性：当前被执行的指令或访问的数据在一段时间内有大概率再次被访问或执行**

   **空间局部性：当前被执行指令附近的指令和被访问数据附近的数据有在一段时间内有大概率被访问到**

   **分支局部性：一条条件跳转语句有大概率下一次还是跳转同一个地址**
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
   ...
   **虚拟存储是使用外存作为程序使用内存的延伸，扩大程序能使用的内存空间。它实在一个进程内部进行协调，并且以使用频率来决定一个页是否留在内存里。好处是管理灵活，使程序运行效率变高，挑战是实现复杂**

2. 虚拟存储需要什么样的支持技术？

   **os实现换入换出，支持硬件的地址转换机制**
...
### 8.5 虚拟页式存储
 ...
 1. 什么是虚拟页式存储？缺页中断处理的功能是什么？
    ...
    **以页为单位进行虚拟内存的换入换出操作。程序加载时，只载入一部分页面，若访问的页不在内存中，则产生缺页异常，将缺页调入内存**

 1. 为了支持虚拟页式存储的实现，页表项有什么修改？

    **加入了驻留位，判断该页是否在内存中；修改位，判断是否被修改，若被修改则在换出时要重新写入外存；访问位，判断是否经常被访问，用于页面置换算法；保护位**

 1. 页式存储和虚拟页式存储的区别是什么？

    **页式存储仅将内存分页，所有进程享用同样的内存空间，而虚拟存储则是每一个进程都有独立的内存空间，并且还要考虑页的换入换出**

### 8.6 缺页异常

1. 缺页异常的处理流程？

   **产生缺页中断->查找在外存中的页面->页面换入->页表项修改->重新执行指令**
