...
2. 不同特权级有什么区别？
   ...
   **低特权级无法访问、修改搞特权级的代码和数据，一些指令只能在高特权级运行**

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

   **若是产生了中断、异常、系统调用，当前的CPL应高于或等于DPL才能进行这次中断，指令执行时的访存需要MAX(CPL, RPL) <= DPL才能进行这次访存。**
...
1. 一条指令在执行时会有哪些可能的特权级判断？
   ...
   **判断代码段的特权级，判断数据段的特权级，判断中断的特权级**

2. 在什么情况下会出现特权级切换？

   **用户态产生中断时会产生特权级切换，或者主动调用特权级切换的函数**

3. int指令在ring0和ring3的执行行为有什么不同？

   **在ring0产生中断在原地压栈，无特权级的转换，ring3产生中断特权级转换为ring0，然后再在内核栈上压栈**

4. 如何利用int和iret指令完成不同特权级的切换？

   **int时将CS、DS、SS、ES等段寄存器压栈，iret返回前对它们进行修改，iret返回后即实现了特权级转换**

5. TSS和Task Register的作用是什么？

   **存储ring0、1、2堆栈的位置，便于在切换特权级时能够找到相应特权级的堆栈**
...
2. 分析页表的建立过程；
   ...
   **Allocate a page as directory table -> Clear the page allocated -> Map 0xc0000000-0xF8000000(va) to 0x00000000-0x38000000 -> Map 0x00000000-0x00100000(va) to 0x00000000-0x00100000(pa) -> 0x00000000-0x00100000 -> Set CR3 & bit 31 of CR0 -> Unmap 0x00000000-0x00100000**
